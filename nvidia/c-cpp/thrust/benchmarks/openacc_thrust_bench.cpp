...
bool run_end_to_end_pipeline(
  ...
  bool ok = !measure ||
            (repeat_index == 0
                ...
                GPU_SUITE_OK;
#pragma acc data copyin(values[0 : count])
  {
    if (ok) {
      reduced_value = transform_reduce(values, count);
...
#pragma acc data copyin(values_ptr[0 : count])
        ...
        for (int trial = 0; trial < options.trials && setup_ok; ++trial) {
          ...
          if (ok) {
            ...
            result.message =
                pass ? ""
                     : (constructed
                            ? "OpenACC Thrust verification failed"
      ...
      if (!warmup_ok) {
        gpu_suite::emit_unmeasured(
            ...
            "OpenACC Thrust end-to-end warmup failed");
        any_failure = true;
      }
    ...
    std::fprintf(stderr, "OpenACC Thrust host vector length is unsupported\n");
    gpu_suite::emit_unmeasured(writer, options, 0, true, "benchmark",
                               "host vector length exceeds max_size");
    any_failure = true;
  } catch (const std::bad_alloc &) {
    std::fprintf(stderr, "OpenACC Thrust host allocation failed\n");
    gpu_suite::emit_unmeasured(writer, options, 0, true, "benchmark",
