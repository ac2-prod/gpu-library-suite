...
ソース範囲は`nvidia/c-cpp`と、追加した[NVIDIA Fortran実装](nvidia/fortran/README.ja.md)です。FortranのGPU実行は未確認で、ローカル検証の範囲を別途明記しています。AMDとその他のシステムへの対応は今後の対象とし、空の仮実装は置いていません。本プロジェクトは`ac2-prod`で管理しており、HAIRDESCの公式リポジトリではありません。
...
Fortran追加も同じコードのみの範囲を維持します。この実装作業自体では新しい版の公開やFortran測定データの配布は行いません。
...
Fortranは[コード対応・ビルド・自己測定の案内](nvidia/fortran/README.ja.md)（[English](nvidia/fortran/README.md)）から進めます。以下の入門経路とソース表は既存C/C++版を対象とし、コンパイラ条件をそのままFortranへ流用しません。
