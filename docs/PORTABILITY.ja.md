...
以下の既定build・コマンド例はC/C++用です。[追加したFortran案内](../nvidia/fortran/README.ja.md)では専用コンパイラ/providerのbuildと診断configを示し、その後のruntime取得・検証・集計・図生成を本ガイドと共用します。言語別にbuild tree・manifest・runを分離してください。FortranのGPU実行は未確認です。
