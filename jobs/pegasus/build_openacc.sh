...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "unknown build_openacc.sh option: $1" >&2; exit 2 ;;
  esac
done
if [[ -z "${config}" || -z "${source_root}" || -z "${build_root}" ]]; then
  echo "usage: build_openacc.sh --config FILE --source DIR --build DIR" >&2
  exit 2
fi
...
nvhpc_cuda_home="$("${helper[@]}" get nvhpc_cuda_home)"
...
printf 'OpenACC partial manifest: %s\n' "${build_root}/partial-manifest.json"
